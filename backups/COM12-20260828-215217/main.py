...
#
# The display is composed in portrait: 64 wide (u) x 128 tall (v).
# Every glyph is upright in this frame; the physical unit stands on its
# long edge. Native mapping: pixel(u,v) -> oled(v, 63-u).
#
#   - right edge (u 48..63): the hardware yellow zone — the name band,
#     3x5 glyphs stacked top -> bottom, black on yellow
#   - the rest (u 0..47): three areas stacked down the screen — CPU /
#     GPU / MEM — each with big digits from a real display font
#     (digits_bebas.py; falls back to a 4x7 table scaled 4x) and a
#     3x5 label
#   - between areas: 1-px pulse dividers, lit from the name band
#     leftward by the audio.level fast lane (attack instant, decay
#     exponential — the VU pattern)
#
# Frames (suzu/1 suzu-t; newline-terminated; `*hh` checksums stripped
# and verified where present — a bad checksum is dropped, state
# self-heals on the next frame):
#   I                     -> OK,{descriptor}*hh   (session)
#   K                     -> OK                    (keepalive; wakes the face)
#   G,report,<cpu>,<mem>,<gpu>  -> ground.set; slots in declared order,
#                          255 = not measured -> drawn as a dash
#   A,audio.level,<0-100> -> fast atom -> the pulse dividers
#   J,{json}              -> context escape; "name" sets the band
#   S,<name>              -> compat alias for the band (dev convenience)
#   X                     -> OK (ring/run overlay restore; nothing overlaid yet)
#   R,<signal>,<urgency>,<hue>,<arc>,<seq>[,<label>]  -> a ring: the
#                          band answers with one blink; ack echoes seq
#
# Ten seconds without frames -> the face rests (contrast dims). Any
# frame wakes it. Honestly quiet when the house is gone.
...
# ── numerals: sprites from a display font, or the 4x7 fallback ──

FALLBACK_4x7 = {
    "0": (6, 9, 9, 9, 9, 9, 6), "1": (2, 6, 2, 2, 2, 2, 7),
    "2": (14, 1, 1, 6, 8, 8, 15), "3": (14, 1, 1, 6, 1, 1, 14),
    "4": (1, 3, 5, 9, 15, 1, 1), "5": (15, 8, 14, 1, 1, 9, 6),
    "6": (6, 8, 8, 14, 9, 9, 6), "7": (15, 1, 1, 2, 2, 4, 4),
    "8": (6, 9, 6, 9, 9, 9, 6), "9": (6, 9, 9, 7, 1, 1, 6),
    "-": (0, 0, 0, 15, 0, 0, 0),
}

try:
    from digits_bebas import DIGITS as _SPR, H as _SPR_H
    NUM_H = _SPR_H                     # 32 — the real font's cap height
except ImportError:
    _SPR = dict((ch, (4, 7, rows)) for ch, rows in FALLBACK_4x7.items())
    NUM_H = 28                         # 7 rows x scale 4
